...
"""
# TODO: Pass Julia vargs instead of Dict.
# TODO: Check whether the R variables are set in the callers scope as well.
function rplot(plot, filename; path_prefix=nothing, uri_prefix=nothing, kwargs=nothing)::String
    im_path = path_prefix != nothing ? joinpath(path_prefix, filename) : filename
    args = merge(Dict("filename" => im_path, "plot" => plot), kwargs)
